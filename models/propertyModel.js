...
const search = (obj, key) => {
    ...
    while (queue.length > 0) {
        ...
        } else if (typeof current === 'object' && current !== null) {// 如果当前对象是普通对象，则遍历它的键值对
            const keys = Object.keys(current);
            for (let v of keys) {
                if (v === key) {
                    return current[v];  // 找到目标键并返回其值
                } else if (v === 'address' || v === 'location' || v === 'ordinates') {
...
module.exports = { requestCatAPI, requestRealtorAPI, requestLocalJSON, DFS };